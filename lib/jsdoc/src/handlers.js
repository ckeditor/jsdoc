...
exports.attachTo = parser => {
    ...
    parser.on('jsdocCommentFound', e => {
        ...
        for (let i = 0, l = comments.length; i < l; i++) {
			...
			// console.log( newDoclet.name, newDoclet.kind, e.parentNode.type );

			// Add the correct memberof for events inside classes.
			// if ( !newDoclet.memberof && newDoclet.kind === 'event' ) {
			// 	if( e.parentNode.type === 'MethodDefinition' ) {
			// 		newDoclet.addTag('instance');
			// 		newDoclet.addTag('memberof', parser.resolveThis( e.parentNode.parent ) );
			// 	}
			// }

			// Add the correct memberof for comments inside classes.
			if ( !newDoclet.memberof && newDoclet.kind === 'member' ) {
				if( e.parentNode.type === 'MethodDefinition' ) {
					newDoclet.addTag('instance');
					newDoclet.addTag('memberof', parser.resolveThis( e.parentNode.parent ) );
				}
