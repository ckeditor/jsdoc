...
walkers[ Syntax.LetStatement ] = ( node, parent, state, cb ) => {
	for ( const headItem of node.head ) {
...
walkers[ Syntax.MethodDefinition ] = ( node, parent, state, cb ) => {
	...
	if ( node.value ) {
		...
		// Move trailing comments to the method definition trailing comments.
		if ( node.value.body && node.value.body.body.length ) {
			const bodyNodes = node.value.body.body;

			moveTrailingComments( bodyNodes[ bodyNodes.length - 1 ], node );
		}
	}
...
walkers[ Syntax.SwitchCase ] = ( node, parent, state, cb ) => {
	...
	for ( const consequentItem of node.consequent ) {
...
class Walker {
	...
	_recurse( filename, ast ) {
		...
		function cb( node, parent, cbState ) {
			...
			const currentScope = getCurrentScope( cbState.scopes );
