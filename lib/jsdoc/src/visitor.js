...
class Visitor {
	...
	visitNodeComments( node, parser, filename ) {
		...
		// Add comments for trailing members inside method definitions.
		if ( node.type === Syntax.MethodDefinition && node.trailingComments && node.trailingComments.length ) {
			// TODO.
			addComments( node.trailingComments );
		}
		...
		for ( let comment of comments ) {
			...
			if ( isValidJsdoc( rawComment ) ) {
				...
				e = new JsdocCommentFound( comment, rawComment, filename, { parentNode } );
		...
		function getParentNode( node ) {
			...
			while ( notMeaningfulNodeType.includes( node.type ) ) {
				node = node.parent;
			}
